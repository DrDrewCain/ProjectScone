...
class Client:
    ...
    async def get_collection(self, name):
        return SimpleNamespace(config=SimpleNamespace(params=SimpleNamespace(vectors=SimpleNamespace(size=self.dim))),
...
@pytest.mark.qdrant
async def test_real_server_repairs_partial_schema_and_preserves_points():
    import os
    ...
    try:
        ...
        index = QdrantVectorIndex(collection=collection, client=client)
        await index.ensure(3)
        after = await client.get_collection(collection)
        assert {name: info.data_type for name,info in after.payload_schema.items()} == EXPECTED
