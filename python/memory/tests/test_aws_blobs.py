...
async def test_hold_capacity_is_checked_before_upload(aws: tuple[AwsClient, AwsClient]) -> None:
    ...
    with pytest.raises(SconeError, match="capacity"):
        await store(aws).put("alpha", b"no capacity", "text/plain")
    assert metadata(aws, "I#alpha") == []
    assert aws[0].list_object_versions(Bucket="scone-test-blobs").get("Versions", []) == []
