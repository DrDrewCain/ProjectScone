...
def test_the_known_step_brings_a_v5_file_forward_and_keeps_everything(tmp_path):
    ...
    assert store.conn.execute("SELECT count(*) FROM sqlite_master WHERE name = 'ingest_jobs'").fetchone()[0] == 1
    after = snapshot(path)
    ...
    sixth = tmp_path / "live.db.v10.bak"
    assert sixth.exists() and snapshot(sixth) == before, "and one before the sixth"
    assert schema_version(again.conn) == 11 and not (tmp_path / "live.db.v11.bak").exists()
...
def test_other_versions_are_still_refused_not_rewritten(tmp_path):
    for version in ("2", "4", "12"):  # 12 stands for a build newer than this one
