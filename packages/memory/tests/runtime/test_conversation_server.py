...
def launched(tmp_path):
    ...
    def start(*options):
        ...
        env["PYTHONPATH"] = os.pathsep.join([root, str(TESTS_ROOT), env.get("PYTHONPATH", "")])
...
def test_configured_launcher_runs_native_scone_without_browser_pages(launched):
    client, _ = launched("--model-factory", "tests.conversations.test_text_conversation:ScriptedModel")
    with closing(client):
