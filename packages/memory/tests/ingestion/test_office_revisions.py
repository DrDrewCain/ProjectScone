...
from .test_office_formats import O, T, TABLE, W, S, R, REL, archive, ooxml_archive
...
async def test_docx_tracks_current_moves_and_omits_hidden_runs_and_properties():
    data = ooxml_archive({'word/document.xml': f'''<w:document xmlns:w="{W}"><w:body>
        ...
        </w:body></w:document>'''}, main_part='word/document.xml')
...
async def test_docx_keeps_nonbreaking_hyphens_position_tabs_and_ruby_base_text():
    data = ooxml_archive({'word/document.xml': f'''<w:document xmlns:w="{W}"><w:body><w:p>
        ...
        </w:p></w:body></w:document>'''}, main_part='word/document.xml')
    parsed = await BuiltinDocumentParser().parse(data, 'source.docx')
...
async def test_docx_table_revision_filter_keeps_explicitly_visible_runs(namespace, visible):
    data = ooxml_archive({'word/document.xml': f'''<w:document xmlns:w="{namespace}"><w:body>
        ...
        </w:p></w:tc></w:tr></w:tbl></w:body></w:document>'''}, main_part='word/document.xml')
    parsed = await BuiltinDocumentParser().parse(data, 'source.docx')
...
async def test_docx_revision_names_in_other_namespaces_do_not_hide_content():
    data = ooxml_archive({'word/document.xml': f'''<w:document xmlns:w="{W}" xmlns:x="urn:example"><w:body><w:p>
