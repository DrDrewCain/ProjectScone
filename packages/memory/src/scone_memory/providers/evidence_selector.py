    ...
    async def select(self, question: str, cards: tuple[EvidenceCard, ...]) -> EvidenceSelection:
        ...
        try:
            response = await self._chat.complete_structured(
                ...
                "Use claims, path_fact_ids and path_steps to identify which recorded route each card represents; "
                "a broad quotation may describe several different routes. Claim triples and their origin identify "
                "stored records, not proof that the quotation entails them. A subject_object step is only a host "
                "structural match of one record's object to the next subject, not a stated causal relationship. "
                "Stored relation direction and traversal are distinct; do not reverse their meaning. "
